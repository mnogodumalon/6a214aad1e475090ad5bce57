...
import type { Patienten, Terminbuchungen, CreatePatienten, CreateTerminbuchungen } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getPatienten(): Promise<Patienten[]> {
    ...
    return enrichLookupFields(records, 'patienten');
  }
  static async getPatientenEntry(id: string): Promise<Patienten | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Patienten;
    return enrichLookupFields([record], 'patienten')[0];
  }
  static async createPatientenEntry(fields: CreatePatienten) {
    return callApi('POST', `/apps/${APP_IDS.PATIENTEN}/records`, { fields: cleanFieldsForApi(fields as any, 'patienten') });
  }
  static async updatePatientenEntry(id: string, fields: Partial<CreatePatienten>) {
    return callApi('PATCH', `/apps/${APP_IDS.PATIENTEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'patienten') });
  }
  ...
  // --- TERMINBUCHUNGEN ---
  static async getTerminbuchungen(): Promise<Terminbuchungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TERMINBUCHUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Terminbuchungen[];
    return enrichLookupFields(records, 'terminbuchungen');
  }
  static async getTerminbuchungenEntry(id: string): Promise<Terminbuchungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TERMINBUCHUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Terminbuchungen;
    return enrichLookupFields([record], 'terminbuchungen')[0];
  }
  static async createTerminbuchungenEntry(fields: CreateTerminbuchungen) {
    return callApi('POST', `/apps/${APP_IDS.TERMINBUCHUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'terminbuchungen') });
  }
  static async updateTerminbuchungenEntry(id: string, fields: Partial<CreateTerminbuchungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.TERMINBUCHUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'terminbuchungen') });
  }
  static async deleteTerminbuchungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TERMINBUCHUNGEN}/records/${id}`);
  }
