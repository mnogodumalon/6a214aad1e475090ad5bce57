...
import { PatientenViewDialog } from '@/components/dialogs/PatientenViewDialog';
import { TerminbuchungenDialog } from '@/components/dialogs/TerminbuchungenDialog';
import { TerminbuchungenViewDialog } from '@/components/dialogs/TerminbuchungenViewDialog';
...
const TERMINBUCHUNGEN_FIELDS = [
  { key: 'patient', label: 'Patient', type: 'applookup/select', targetEntity: 'patienten', targetAppId: 'PATIENTEN', displayField: 'vorname' },
  { key: 'termin', label: 'Termin (Datum und Uhrzeit)', type: 'date/datetimeminute' },
  { key: 'behandlungsraum', label: 'Behandlungsraum', type: 'lookup/radio', options: [{ key: 'raum_2', label: 'Raum 2' }, { key: 'raum_3', label: 'Raum 3' }, { key: 'raum_1', label: 'Raum 1' }] },
  { key: 'bemerkungen', label: 'Bemerkungen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'patienten', label: 'Patienten', pascal: 'Patienten' },
  { key: 'terminbuchungen', label: 'Terminbuchungen', pascal: 'Terminbuchungen' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('patienten');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'patienten': new Set(),
    'terminbuchungen': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'patienten': return (data as any).patienten as Patienten[] ?? [];
      case 'terminbuchungen': return (data as any).terminbuchungen as Terminbuchungen[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'terminbuchungen':
        lists.patientenList = (data as any).patienten ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'terminbuchungen' && fieldKey === 'patient') {
      const match = (lists.patientenList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'patienten': return PATIENTEN_FIELDS;
      case 'terminbuchungen': return TERMINBUCHUNGEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'patienten': return {
        create: (fields: any) => LivingAppsService.createPatientenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updatePatientenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deletePatientenEntry(id),
      };
      case 'terminbuchungen': return {
        create: (fields: any) => LivingAppsService.createTerminbuchungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTerminbuchungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTerminbuchungenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'patienten' || dialogState?.entity === 'patienten') && (
        <PatientenDialog
          open={createEntity === 'patienten' || dialogState?.entity === 'patienten'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'patienten' ? handleUpdate : (fields: any) => handleCreate('patienten', fields)}
          defaultValues={dialogState?.entity === 'patienten' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Patienten']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Patienten']}
        />
      )}
      {(createEntity === 'terminbuchungen' || dialogState?.entity === 'terminbuchungen') && (
        <TerminbuchungenDialog
          open={createEntity === 'terminbuchungen' || dialogState?.entity === 'terminbuchungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'terminbuchungen' ? handleUpdate : (fields: any) => handleCreate('terminbuchungen', fields)}
          defaultValues={dialogState?.entity === 'terminbuchungen' ? dialogState.record?.fields : undefined}
          patientenList={(data as any).patienten ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Terminbuchungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Terminbuchungen']}
        />
      )}
      {viewState?.entity === 'patienten' && (
        <PatientenViewDialog
          open={viewState?.entity === 'patienten'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'patienten', record: r }); }}
        />
      )}
      {viewState?.entity === 'terminbuchungen' && (
        <TerminbuchungenViewDialog
          open={viewState?.entity === 'terminbuchungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'terminbuchungen', record: r }); }}
          patientenList={(data as any).patienten ?? []}
        />
      )}
