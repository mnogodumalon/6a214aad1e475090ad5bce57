...
import type { Terminbuchungen, Patienten } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Terminbuchungen';
...
interface TerminbuchungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Terminbuchungen['fields']) => Promise<void>;
  defaultValues?: Terminbuchungen['fields'];
...
export function TerminbuchungenDialog({ open, onClose, onSubmit, defaultValues, recordId, patientenList, enablePhotoScan = true, enablePhotoLocation = true }: TerminbuchungenDialogProps) {
  const [fields, setFields] = useState<Partial<Terminbuchungen['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Terminbuchungen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'terminbuchungen');
      await onSubmit(clean as Terminbuchungen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["patient"]);
        ...
        return merged as Partial<Terminbuchungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Terminbuchungen bearbeiten' : 'Terminbuchungen hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'patient': (
      <div key="patient" className="space-y-1.5">
        ...
        <Combobox
          ...
          createLabel="Neu in Patienten"
        />
      </div>
    ),
    'termin': (
      <div key="termin" className="space-y-1.5">
        <Label htmlFor="termin">Termin (Datum und Uhrzeit)</Label>
        <DatePicker
          id="termin"
    ...
    'behandlungsraum': (
      <div key="behandlungsraum" className="space-y-1.5">
        <Label htmlFor="behandlungsraum">Behandlungsraum</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.behandlungsraum) === 'raum_2'}
            onClick={() => setFields(f => ({ ...f, behandlungsraum: (lookupKey(f.behandlungsraum) === 'raum_2' ? undefined : 'raum_2') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.behandlungsraum) === 'raum_2'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.behandlungsraum) === 'raum_3'}
            onClick={() => setFields(f => ({ ...f, behandlungsraum: (lookupKey(f.behandlungsraum) === 'raum_3' ? undefined : 'raum_3') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.behandlungsraum) === 'raum_3'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.behandlungsraum) === 'raum_1'}
            onClick={() => setFields(f => ({ ...f, behandlungsraum: (lookupKey(f.behandlungsraum) === 'raum_1' ? undefined : 'raum_1') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.behandlungsraum) === 'raum_1'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'bemerkungen': (
      <div key="bemerkungen" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkungen"
          placeholder="Besonderheiten, allergische Reaktionen, Fasting-Status..."
  ...
  const FIELD_LABELS: Record<string, string> = {"patient": "Patient", "termin": "Termin (Datum und Uhrzeit)", "behandlungsraum": "Behandlungsraum", "bemerkungen": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"patient": {"vorname": "Vorname", "nachname": "Nachname", "geburtsdatum": "Geburtsdatum", "telefonnummer": "Telefonnummer", "email": "E-Mail-Adresse"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.TERMINBUCHUNGEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
