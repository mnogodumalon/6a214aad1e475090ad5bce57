...
interface PatientenViewDialogProps {
  open: boolean;
  onClose: () => void;
  record: Patienten | null;
  onEdit: (record: Patienten) => void;
}

export function PatientenViewDialog({ open, onClose, record, onEdit }: PatientenViewDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Patienten anzeigen</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="pt-2 border-t border-border">
            <AttachmentsSection appId={APP_IDS.PATIENTEN} recordId={record.record_id} readOnly />
          </div>
        </div>
      </DialogContent>
    </Dialog>
