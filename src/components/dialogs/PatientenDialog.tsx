...
import type { Patienten } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Patienten';
...
interface PatientenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Patienten['fields']) => Promise<void>;
  defaultValues?: Patienten['fields'];
...
export function PatientenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: PatientenDialogProps) {
  const [fields, setFields] = useState<Partial<Patienten['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Patienten['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'patienten');
      await onSubmit(clean as Patienten['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Patienten['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Patienten bearbeiten' : 'Patienten hinzufügen';
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        ...
        <Input
          id="email"
          type="email"
          placeholder="z. B. anna.mueller@example.com"
          value={fields.email ?? ''}
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "geburtsdatum": "Geburtsdatum", "telefonnummer": "Telefonnummer", "email": "E-Mail-Adresse"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.PATIENTEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
